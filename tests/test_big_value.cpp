...
int main() {
  ...
  assert(found);
  compare(data, read_l0.data(), l0, "l0");

  auto read_l1 = tx.read("l1", found);
  assert(found);
  compare(data, read_l1.data(), l1, "l1");

  auto read_l1_1 = tx.read("lx", found);
  assert(found);
  compare(data, read_l1_1.data(), l1 + 1, "l1.1");

  auto read_l2 = tx.read("l2", found);
  assert(found);
  compare(data, read_l2.data(), l2, "l2");

  // Test partial reads

  auto read_l2_edge_start = tx.read_part("l2", 0, 20, found);
  compare(data + 0, read_l2_edge_start.data(), 20, "l2_edge_start");

  auto read_l2_boundary_1 = tx.read_part("l2", l0 - 10, 20, found);
  compare(data + l0 - 10, read_l2_boundary_1.data(), 20, "l2_boundary_1");

  auto read_l2_first_two = tx.read_part("l2", 1, l1, found);
  compare(data + 1, read_l2_first_two.data(), l1, "l2_first_two");

  auto read_l2_boundary_2 = tx.read_part("l2", l1 - 10, 20, found);
  compare(data + l1 - 10, read_l2_boundary_2.data(), 20, "l2_boundary_2");

  auto read_l2_edge_end = tx.read_part("l2", l2 - 10, 10, found);
  compare(data + l2 - 10, read_l2_edge_end.data(), 10, "l2_edge_end");
