...
{
    {
        diskmap::DiskMap dm("test_persistence.dm");
        dm.write("persist_key", "persist_value", 14);
    } // Close database

    diskmap::DiskMap dm("test_persistence.db");
    bool found;
    auto result = dm.read("persist_key", found);
    assert(found);
    assert(std::string(result.data_ptr(), result.size()) == "persist_value");

    print_test_result("Persistence", true);
}
